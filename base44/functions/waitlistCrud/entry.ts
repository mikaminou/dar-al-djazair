...
function mapRow(row) {
  ...
  return {
    ...
    listing_wilaya: row.listing_wilaya,
...
Deno.serve(async (req) => {
  try {
    ...
    if (operation === 'list') {
      ...
      } else q = q.order('joined_at', { ascending: true });
    ...
    if (operation === 'create') {
      const insert = { ...pickFields(data || {}), user_email: data?.user_email || user.email };
      // Auto-assign next position if not provided
      if (insert.position === undefined && insert.listing_id) {
        const { count } = await sb.from(TABLE).select('id', { count: 'exact', head: true }).eq('listing_id', insert.listing_id);
        insert.position = (count || 0) + 1;
      }
      if (!insert.joined_at) insert.joined_at = new Date().toISOString();
