...
const TABLE = 'appointment_proposals';
const ALLOWED_FIELDS = [
  'thread_id', 'listing_id', 'listing_title', 'proposer_email', 'other_email',
  'proposer_name', 'proposed_date', 'proposed_start_time', 'proposed_end_time',
  'status', 'notes'
];
...
Deno.serve(async (req) => {
  try {
    ...
    if (operation === 'create') {
      const insert = { ...pickFields(data || {}), proposer_email: data?.proposer_email || user.email };
