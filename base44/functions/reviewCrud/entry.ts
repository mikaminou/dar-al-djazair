...
function mapRow(row) {
  ...
  return {
    ...
    reviewed_email: row.reviewed_email,
    lead_id: row.lead_id,
...
Deno.serve(async (req) => {
  try {
    ...
    if (operation === 'create') {
      const insert = { ...pickFields(data || {}), reviewer_email: data?.reviewer_email || user.email };
