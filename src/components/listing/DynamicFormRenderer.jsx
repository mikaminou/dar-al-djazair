...
function NumberField({ field, value, onChange, error, warning, lang, listingType }) {
   ...
   if (isAmenity) {
     ...
     return (
       <div>
         ...
         <div className="flex items-center gap-2 border border-gray-200 rounded-full px-2.5 py-2 w-fit bg-white hover:border-emerald-300 transition-colors">
           <button
             type="button"
             onClick={() => onChange(field.key, Math.max((field.min ?? 0), numVal - 1))}
             disabled={numVal <= (field.min ?? 0)}
             className="w-6 h-6 flex items-center justify-center rounded-full text-sm font-bold text-emerald-600 hover:bg-emerald-50 disabled:text-gray-300 disabled:hover:bg-transparent transition-colors"
           >
             −
           </button>
           <span className="text-sm font-semibold text-gray-700 min-w-[28px] text-center">{numVal}</span>
           <button
             type="button"
  ...
  return (
    <div>
      ...
      {error && <p className="flex items-center gap-1 text-xs text-red-600 mt-2"><AlertCircle className="w-3 h-3" />{error}</p>}
      {warning && !error && <p className="flex items-center gap-1 text-xs text-amber-600 mt-2"><AlertTriangle className="w-3 h-3" />{warning}</p>}
    </div>
  );
}
