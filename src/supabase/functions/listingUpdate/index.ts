/**
 * listingUpdate — partial update of a listing. Requires auth.
 * Payload: { id: string, data: <partial listing object> }
 */
...
function extractTypeFields(propertyType: string, data: any) {
  ...
  for (const col of cfg.columns) {
    ...
    if (merged[col] !== undefined) out[col] = merged[col];
  }
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!id || !data) return Response.json({ error: "id and data required" }, { status: 400, headers: corsHeaders });

    const sb = getServiceClient();

    const { data: existing, error: fetchErr } = await sb
      .from("listings").select("id, owner_id").eq("id", id).maybeSingle();
